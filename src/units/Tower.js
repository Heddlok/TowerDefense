...
export class Tower {
  ...
  // Centralized base stats table (also used by static pricing helpers)
  static STATS = {
    basic: {
      range: 140,
      fireRate: 0.8,
  ...
  static getTowerStats(type) {
    return Tower.STATS[type] || Tower.STATS.basic;
  }

  /** Returns the multiplier based on how many of this type exist now (pre-purchase). 
   *  +42% for every 2 towers already owned: 0–1 -> 1.00x, 2–3 -> 1.42x, 4–5 -> 1.84x, etc. */
  static getPriceMultiplier(type, countOverride = null) {
    const count = (countOverride ?? Tower.towerCounts[type] ?? 0);
    const steps = Math.floor(count / 2);
    return 1 + Tower.SECOND_BUMP * steps;
  }

  /** UI: price to buy the *next* tower of this type (given current counts). */
  static getNextTowerCost(type) {
    const base = Tower.getTowerStats(type).cost;
    const mult = Tower.getPriceMultiplier(type);
    return Math.floor(base * mult);
  }

  /** UI: preview price after n more purchases (e.g., n=1 to show “price after this one”). */
  static getFutureTowerCost(type, n = 1) {
    const base = Tower.getTowerStats(type).cost;
    const current = Tower.towerCounts[type] ?? 0;
    const futureMult = Tower.getPriceMultiplier(type, current + n);
    return Math.floor(base * futureMult);
  }
  ...
  // ===== Instance =====
  constructor(tx, ty, type = 'basic', purchaseCost = null, incrementCount = true) {
    this.tx = tx;
    this.ty = ty;
    ...
    // Stats from table
    const stats = Tower.getTowerStats(type);
    ...
    // Use dynamic purchase cost unless provided (lets you override from outside if needed)
    const dynamicCost = (purchaseCost != null) ? purchaseCost : Tower.getNextTowerCost(this.type);
    this.cost = dynamicCost;          // cost paid for this tower (useful if you show it in UI)
    this.totalCost = dynamicCost;     // tracks money invested into this tower (incl. upgrades later)

    // After creation, this tower counts toward future pricing
    if (incrementCount) {
      Tower.towerCounts[this.type] = (Tower.towerCounts[this.type] ?? 0) + 1;
    }
