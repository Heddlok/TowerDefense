...
export function getWavePlan(wave) {
  ...
  const count = Math.floor(8 + wave * 0.9 + Math.pow(Math.max(0, wave - 6), 1.12) * 0.12);
  ...
  return {
    ...
    // A gentle “pressure spike” every 5 waves
    burstEvery: 5,
    burstBonus: (wave % 5 === 0) ? Math.max(2, Math.floor(wave / 5)) : 0,
  };
}
